        ...
        if df is not None and not df.empty:
            for _, row in df.iterrows():
                ...
                if "_" in ch:
                    ...
                    g_data = parti[0]
                    if g_data in dati:
                        # Ricostruisce la fascia oraria originale basandosi sulla chiave pulita
...
with tab1:
    ...
    if st.button("🖨️ Genera PDF Tabellone Settimanale", key="gen_pdf_tab_btn", use_container_width=True):
        ...
        t_table = Table(data_pdf, colWidths=[90, 85, 85, 85, 85, 85, 85, 85])
...
if st.button("📊 Genera PDF Report Ore", key="gen_pdf_rep_btn", use_container_width=True):
    ...
    t_rep = Table(data_pdf, colWidths=[160, 50, 50, 50, 120, 70])
